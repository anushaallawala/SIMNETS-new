function [varargout] = plotNTmovie(NT,id,varargin)
...
kw = getopt(['colors symbol=''o'' lwidth=2 msize=5 fhandle ahandle add_legend=1 alpha=.2'...
...
for gon=(ugid)
	...
	if ndim==2
		...
		if (tsteps > 1) && kw.use_patchline
			...
			y = y(plot_times, :);
			for i_h = 1:size(x, 2)
				h(i_h) = patchline(x(:,i_h), y(:,i_h),   'linewidth',kw.lwidth, 'parent', kw.ahandle, 'facecolor', kw.colors(classcount,:),'EdgeColor', kw.colors(classcount,:),'edgealpha', kw.alpha);
			end
		else
		...
		if (tsteps > 1) && kw.use_patchline
			...
			for i_h = 1:size(x, 2)
				h(i_h) = patchline(x(:,i_h), y(:,i_h), z(:,i_h),'linewidth',kw.lwidth, 'parent', kw.ahandle, 'facecolor', kw.colors(classcount,:),'EdgeColor', kw.colors(classcount,:),'edgealpha', kw.alpha);
			end
		else
